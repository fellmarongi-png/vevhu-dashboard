...
export function WorkerForm({ defaultValues, onSuccess }: WorkerFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      daily_target: defaultValues?.daily_target != null ? String(defaultValues.daily_target) : "10",
  ...
  async function onSubmit(values: z.output<typeof workerSchema>) {
    ...
    if (isEdit && defaultValues?.id) {
      ...
      const { error } = await supabase.from("users").insert([
        { ...payload, role: "worker", is_active: true },
      ]);
      if (error) {
        setServerError(error.message);
        return;
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="space-y-1">
        ...
        {errors.zone_assigned && (
          <p className="text-xs text-destructive">{errors.zone_assigned.message}</p>
        )}
      </div>
      ...
      <div className="space-y-1">
        ...
        />
        {errors.daily_target && (
          <p className="text-xs text-destructive">{errors.daily_target.message}</p>
        )}
      </div>
