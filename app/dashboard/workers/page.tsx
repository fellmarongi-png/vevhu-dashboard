...
import { WorkerForm, type WorkerDefaults } from "@/components/workers/worker-form";
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import { PlusIcon, PencilIcon, PowerIcon, AlertCircle } from "lucide-react";
...
export default function WorkersPage() {
  ...
  async function fetchWorkers() {
    ...
    setWorkers(
      data.map((w) => ({ ...w, today_count: countMap[w.id] ?? 0 }))
    );
    setLoading(false);
  }
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      <div className="flex flex-col gap-2 sm:gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold tracking-tight">Workers</h1>
          <p className="text-xs sm:text-sm text-muted-foreground">Manage field workers and their targets.</p>
        </div>
                  ...
                  workers.map((worker) => {
                    ...
                    return (
                      <TableRow key={worker.id}>
                        ...
                        <TableCell className="hidden sm:table-cell">{worker.zone_assigned ?? "—"}</TableCell>
                        <TableCell>
                          {worker.today_count ?? 0} / {worker.daily_target ?? "—"}
                        </TableCell>
