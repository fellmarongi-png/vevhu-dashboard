...
import { PlusIcon, MegaphoneIcon, Trash2Icon } from "lucide-react";
...
export default function AnnouncementsPage() {
  ...
  async function fetchData() {
    ...
    workerData?.forEach((w) => { if (w.zone_assigned) zoneSet.add(w.zone_assigned); });
  ...
  async function onSubmit(values: AnnouncementFormValues) {
    ...
    try {
      ...
      const { error } = await supabase.from("announcements").insert({
        ...
        target_worker_id: values.target_type === "worker" ? values.target_worker : null,
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold tracking-tight">Announcements</h1>
          <p className="text-xs sm:text-sm text-muted-foreground">Send messages to field agents.</p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetForm(); }}>
          <DialogTrigger render={<Button className="w-full sm:w-auto"><PlusIcon className="w-4 h-4 mr-2" />New Announcement</Button>} />
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-2">
              ...
              {targetType === "worker" && (
                <div className="space-y-1.5">
                  ...
                  {errors.target_worker && (
                    <p className="text-xs text-destructive">{errors.target_worker.message}</p>
                  )}
                </div>
              )}
              ...
              {targetType === "zone" && (
                <div className="space-y-1.5">
                  ...
                  />
                  {errors.target_zone && (
                    <p className="text-xs text-destructive">{errors.target_zone.message}</p>
                  )}
                </div>
              )}
      ...
      <Card>
        <CardContent className="p-2 sm:p-6 pt-2 sm:pt-0">
          {loading ? (
            <p className="text-muted-foreground text-sm py-6">Loading announcements...</p>
          ) : announcements.length === 0 ? (
            <div className="py-10 sm:py-16 text-center">
              <MegaphoneIcon className="w-10 h-10 sm:w-12 sm:h-12 mx-auto mb-3 text-muted-foreground/40" />
              <p className="text-sm text-muted-foreground">No announcements yet. Send your first one!</p>
            </div>
          ) : (
                  ...
                  {announcements.map((ann) => {
                    ...
                    return (
                      <TableRow key={ann.id}>
                        <TableCell>
                          <div>
                            <p className="font-medium text-xs sm:text-sm">{ann.title}</p>
                            <p className="text-[11px] sm:text-sm text-muted-foreground line-clamp-1">{ann.message}</p>
                          </div>
                        </TableCell>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1 sm:gap-2">
                            <span className="text-xs sm:text-sm font-medium">{readCount}/{total}</span>
                            {total > 0 && (
                            ...
                            <span className="text-[10px] sm:text-xs text-muted-foreground">{total > 0 ? `${pct}%` : "—"}</span>
                          </div>
                        </TableCell>
                        <TableCell>
