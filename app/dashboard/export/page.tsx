...
export default function ExportPage() {
  ...
  React.useEffect(() => {
    async function loadFilters() {
      ...
      const { data: zoneData } = await supabase
        ...
        .not("zone", "is", null);
      if (zoneData) {
        const unique = [...new Set(zoneData.map((r) => r.zone as string))].filter(Boolean).sort();
        setZones(unique);
      }
    }
    loadFilters();
  ...
  async function handlePreview() {
    ...
    try {
      ...
      if (dateTo) countQuery = countQuery.lte("collected_at", dateTo + "T23:59:59");
      if (workerFilter !== "all") countQuery = countQuery.eq("worker_id", workerFilter);
  ...
  async function handleDownload() {
    ...
    try {
      ...
      const flatRows = rows.map((r) => {
        ...
        extraKeys.forEach((k) => {
          base[`extra_${k}`] = r.extra_fields ? (r.extra_fields as Record<string, unknown>)[k] : null;
        });
        return base;
      });
      ...
      XLSX.writeFile(wb, `submissions_export_${new Date().toISOString().slice(0, 10)}.xlsx`);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div>
        <h1 className="text-lg sm:text-2xl font-bold tracking-tight">Export Data</h1>
        <p className="text-xs sm:text-sm text-muted-foreground">Filter submissions and download as Excel.</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="p-3 pt-0 sm:p-6 sm:pt-0">
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mt-4 sm:mt-5">
            <Button onClick={handlePreview} disabled={loading} variant="outline" className="w-full sm:w-auto">
        ...
        <Card>
          ...
          <CardContent className="p-3 pt-0 sm:p-6 sm:pt-0">
            {previewData.length === 0 ? (
              <p className="text-muted-foreground text-sm">No submissions match the selected filters.</p>
            ) : (
                ...
                <Table className="text-xs sm:text-sm">
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="hidden sm:table-cell">Collected At</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {previewData.map((row) => (
                      <TableRow key={row.id}>
                        ...
                        <TableCell className="hidden sm:table-cell">{row.zone ?? "—"}</TableCell>
                        <TableCell>
