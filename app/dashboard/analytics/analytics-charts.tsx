...
import { FileText, Users, RefreshCw, AlertTriangle } from "lucide-react";
...
export function AnalyticsCharts({
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div>
        <h1 className="text-lg sm:text-2xl font-bold tracking-tight">Analytics</h1>
        ...
        {kpiCards.map((card) => (
          <Card key={card.title} className="transition-shadow hover:shadow-md">
            <CardContent className="flex flex-col items-center p-3 text-center sm:p-5">
              ...
              <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground sm:text-[11px]">{card.title}</p>
              <p className="mt-1 text-2xl font-bold tabular-nums sm:mt-2 sm:text-3xl">{card.value}</p>
              <p className="mt-1 text-[10px] text-muted-foreground sm:mt-1.5 sm:text-xs">{card.description}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <div className="grid gap-3 sm:gap-4 md:grid-cols-2">
        <Card>
          <CardHeader className="p-3 sm:p-6">
            <CardTitle className="text-sm sm:text-base">Daily Submissions</CardTitle>
        ...
        <Card>
          <CardHeader className="p-3 sm:p-6">
            <CardTitle className="text-sm sm:text-base">Records per Worker</CardTitle>
            <CardDescription className="text-xs">Top 15 workers (all time)</CardDescription>
          </CardHeader>
          <CardContent className="p-3 pt-0 sm:p-6 sm:pt-0">
            <div className="h-[200px] sm:h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <Card>
        <CardHeader className="p-3 sm:p-6">
          <CardTitle className="text-sm sm:text-base">Worker Leaderboard</CardTitle>
          <CardDescription className="text-xs">Top 10 workers by records this week</CardDescription>
        </CardHeader>
        <CardContent className="p-3 pt-0 sm:p-6 sm:pt-0">
