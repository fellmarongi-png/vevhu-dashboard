...
export default function ImportPage() {
  ...
  const [columnMapping, setColumnMapping] = React.useState<Partial<Record<OurFieldKey, string>>>({});
    ...
    try {
      ...
      for (let i = 0; i < records.length; i += batchSize) {
        ...
        const { error, data } = await supabase
          .from("known_stands")
          .upsert(validBatch, { onConflict: "stand_number", ignoreDuplicates: false })
          .select();
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-bold tracking-tight">Import Stands</h1>
        <p className="text-xs sm:text-sm text-muted-foreground">Upload a CSV or Excel file to import known stands data.</p>
      </div>
      ...
      {detectedColumns.length > 0 && (
        <Card>
          <CardHeader className="p-3 sm:p-6">
            <CardTitle className="text-sm sm:text-base">Column Mapping</CardTitle>
          </CardHeader>
          <CardContent className="p-3 pt-0 sm:p-6 sm:pt-0">
            <p className="text-xs sm:text-sm text-muted-foreground mb-4">
              Detected {detectedColumns.length} columns in your file. Map them to our fields below.
            </p>
              ...
              {OUR_FIELDS.map(({ key, label }) => (
                <div key={key} className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setColumnMapping((prev) => ({ ...prev, [key]: v || undefined }))
                    }
                  >
                  ...
                  <TableRow>
                    {OUR_FIELDS.map(({ key, label }) => (
                      <TableHead key={key}>
                        {label}
                        {!columnMapping[key] && (
                          <span className="text-muted-foreground text-xs ml-1">(unmapped)</span>
                        )}
                      </TableHead>
                    ))}
                  </TableRow>
                  ...
                  {previewRows.map((row, i) => (
                    <TableRow key={i}>
                      {OUR_FIELDS.map(({ key }) => {
                        ...
                        return (
                          <TableCell key={key}>
                            {srcCol ? row[srcCol] || "—" : <span className="text-muted-foreground">—</span>}
                          </TableCell>
                        );
                      })}
                    </TableRow>
