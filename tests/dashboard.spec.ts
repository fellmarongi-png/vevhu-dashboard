...
test.describe('Dashboard Overview', () => {
  ...
  test('navigation sidebar is visible with nav items', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: /form builder/i })).toBeVisible();
  });

  test('clicking Submissions nav item navigates to /dashboard/submissions', async ({ page }) => {
    await page.getByRole('link', { name: /submissions/i }).click();
    await expect(page).toHaveURL('/dashboard/submissions');
  });

  test('clicking Map View nav item navigates to /dashboard/map', async ({ page }) => {
    await page.getByRole('link', { name: /map view/i }).click();
    await expect(page).toHaveURL('/dashboard/map');
  });

  test('clicking Analytics nav item navigates to /dashboard/analytics', async ({ page }) => {
    await page.getByRole('link', { name: /analytics/i }).click();
    await expect(page).toHaveURL('/dashboard/analytics');
  });

  test('clicking Form Builder nav item navigates to /dashboard/form-builder', async ({ page }) => {
    await page.getByRole('link', { name: /form builder/i }).click();
    await expect(page).toHaveURL('/dashboard/form-builder');
  });

  test('Recent Submissions and Agent Activity cards are present', async ({ page }) => {
