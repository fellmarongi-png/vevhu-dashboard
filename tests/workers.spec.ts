...
test.describe('Worker Management', () => {
  ...
  test('clicking Add Worker opens dialog with form', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /add worker/i })).toBeVisible();
  ...
  test('submitting the form triggers Add Worker button in dialog', async ({ page }) => {
    await page.getByRole('button', { name: /add worker/i }).click();
    ...
    await expect(page.getByRole('button', { name: /add worker/i }).last()).toBeVisible();
  });
});
