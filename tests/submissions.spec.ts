...
test.describe('Submissions Page', () => {
  ...
  test('renders table with expected columns', async ({ page }) => {
    await expect(page.getByRole('columnheader', { name: /worker/i })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: /stand/i })).toBeVisible();
    await expect(page.getByRole('columnheader', { name: /respondent/i })).toBeVisible();
  ...
  test('clicking a table row navigates to submission detail page', async ({ page }) => {
